...
For aggregated charts (like bar charts showing totals):
- Use 'Brand' as the grouping column
- Sum 'Revenue' and 'Units Sold'
- Average 'Rating'
- Sum 'Reviews'
...
2. charts: A list of chart configurations, where each chart has:
   ...
   - x_column: The column name for the x-axis (e.g., 'Brand')
   - y_column: The column name for the y-axis (e.g., 'Revenue')
...
async def create_chart(
    ...
    **kwargs
    ...
    # For bar and pie charts, aggregate the data
    if chart_type in ['bar', 'pie']:
        if y_column:
            # Group by x_column and calculate the sum/mean based on the y_column
            agg_func = 'mean' if y_column == 'Rating' else 'sum'
            df_agg = df.groupby(x_column)[y_column].agg(agg_func).reset_index()
            print(f"Aggregated data ({agg_func}):")
            print(df_agg.head())
            df = df_agg
    ...
    try:
        ...
        # Update layout
        fig.update_layout(
            template="plotly_dark" if kwargs.get('theme') == 'dark' else "plotly_white",
